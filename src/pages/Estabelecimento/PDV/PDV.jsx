...
/* ════════════════════════════════════════════════════════════
   MODAL DE PAGAMENTO
   ════════════════════════════════════════════════════════════ */
function PagamentoModal({ total, onFinalizar, onCancelar, loading, estabelecimentoId }) {
  ...
  /* ── Foco pós-confirmação do método ──────────────────────── */
  useEffect(() => {
  ...
  /* ── Scroll lista clientes ───────────────────────────────── */
  useEffect(() => {
    if (clienteIndex < 0 || !listaClienteRef.current) return;
    const item = listaClienteRef.current.children[clienteIndex];
  ...
  useEffect(() => {
    ...
    const item = listaMeiosRef.current.children[selectedIndex];
  ...
  async function buscarCliente(termo) {
    ...
    try {
      const resp = await fetch(`${API_URL}/api/clientes/buscar/${estabelecimentoId}?termo=${encodeURIComponent(termo)}`);
  ...
  function handleOverlayKey(e) {
    ...
    if (!metodoConfirmado) {
      if (e.key === 'ArrowDown') {
        ...
        e.preventDefault();
        setSelectedIndex(p => (p - 1 + MEIOS.length) % MEIOS.length);
      } else if (e.key === 'Enter') {
        e.preventDefault();
        confirmarMetodo(MEIOS[selectedIndex].key, selectedIndex);
      }
    }
  }

  /* ── Teclado no input dinheiro ───────────────────────────── */
  function handleDinheiroKey(e) {
    if (e.key === 'Enter') { e.preventDefault(); confirmarFinal(); }
    if (e.key === 'Escape') { e.preventDefault(); onCancelar(); }
  }

  /* ── Teclado na busca de cliente ─────────────────────────── */
  function handleClienteKey(e) {
    if (e.key === 'Escape') { e.preventDefault(); onCancelar(); return; }
    if (e.key === 'ArrowDown') {
      e.preventDefault();
      setClienteIndex(p => Math.min(p + 1, resultadosCliente.length - 1));
    } else if (e.key === 'ArrowUp') {
      e.preventDefault();
      setClienteIndex(p => Math.max(p - 1, 0));
    } else if (e.key === 'Enter') {
      e.preventDefault();
  ...
  return (
    ...
    >
      <div className="pdv-modal" onClick={e => e.stopPropagation()}>
          ...
          <>
            ...
            <ul className="pdv-meios-lista" ref={listaMeiosRef}>
              {MEIOS.map((m, i) => (
                <li
                  key={m.key}
                  className={`pdv-meio-item${selectedIndex === i ? ' ativo' : ''}`}
                  onClick={() => confirmarMetodo(m.key, i)}
                >
                  <span className="pdv-meio-icone">{m.icone}</span>
                  <span style={{ flex: 1 }}>{m.label}</span>
                  {selectedIndex === i && (
                    <span className="pdv-meio-enter">↩ Enter</span>
                  )}
                </li>
              ))}
            </ul>
          ...
          <div className="pdv-pagamento-conteudo">
            ...
            {['Pix', 'Debito', 'Credito'].includes(meioPagamento) && (
              <div className="pdv-pagamento-digital">
                <span className="pdv-pagamento-digital-icone">
                  {MEIOS.find(m => m.key === meioPagamento)?.icone}
                </span>
                <span className="pdv-pagamento-digital-nome">
                  {MEIOS.find(m => m.key === meioPagamento)?.label}
                </span>
              ...
              <>
                {clienteSelecionado ? (
                  <div className="pdv-cliente-selecionado">
                    <span className="pdv-cliente-selecionado-nome">
                    ...
                    <div className="pdv-cliente-selecionado-info">
                      <div className="pdv-cliente-info-item">
                        ...
                        <span className="pdv-cliente-info-valor">
                          {fmt(clienteSelecionado.saldo_devedor)}
                        </span>
                      </div>
                      <div className="pdv-cliente-info-item">
                        <span className="pdv-cliente-info-label">Novo saldo</span>
                        <span className="pdv-cliente-info-valor novo-saldo">
                          {fmt((parseFloat(clienteSelecionado.saldo_devedor) || 0) + total)}
                        </span>
                      </div>
                    </div>
                    <button className="pdv-btn-trocar-cliente" onClick={() => setClienteSelecionado(null)}>
                      ↩ Trocar cliente
                    </button>
                  </div>
                ) : (
                  <>
                    ...
                    />
                    {loadingCliente && (
                      <div style={{ fontSize: '0.78rem', color: 'var(--est-text-muted)', marginBottom: 6 }}>
                        Buscando…
                      </div>
                    )}
                    {resultadosCliente.length > 0 && (
                      <ul className="pdv-cliente-lista" ref={listaClienteRef}>
                        {resultadosCliente.map((cli, i) => (
                          <li
                            key={cli.id}
                            className={`pdv-cliente-item${clienteIndex === i ? ' ativo' : ''}`}
                            onClick={() => selecionarCliente(cli)}
                            onMouseEnter={() => setClienteIndex(i)}
                          >
                            {cli.nome}
                            <span className="pdv-cliente-item-tel">
                              {cli.telefone || '—'}
                            </span>
                          </li>
                        ))}
        ...
        {/* Erro */}
        {erro && (
          <div className="pdv-pagamento-erro">⚠️ {erro}</div>
        )}

        {/* Ações */}
        <div className="pdv-pagamento-acoes">
          <button className="pdv-btn-cancelar" onClick={onCancelar} disabled={loading}>
            Cancelar (Esc)
          </button>
          <button
            ref={btnConfirmarRef}
            className="pdv-btn-confirmar"
...
export default function PDV({ estabelecimentoId }) {
  ...
  useEffect(() => {
    if (!showPagamento && !itemQuantificar && editIndex === null) {
  ...
  async function buscarProdutos(termo) {
    ...
    try {
      const resp = await fetch(
        `${API_URL}/api/estabelecimentoss/${estabelecimentoId}/produtos/buscar-global?termo=${encodeURIComponent(termo)}`
      );
      if (!resp.ok) throw new Error();
  ...
  function selecionarProduto(produto) {
    ...
    if (estoque <= 0) {
      mostrarStatus('erro', `"${produto.nome}" sem estoque!`);
      limparBusca();
      return;
    }

    const qtdNoCarrinho = carrinho
      .filter(i => i.id === produto.id)
  ...
  function confirmarQuantidade(e) {
    ...
    if (editIndex !== null) {
      const outrasQtds = carrinho
        .filter((item, idx) => item.id === produto.id && idx !== editIndex)
        .reduce((acc, i) => acc + i.quantidade, 0);

      if (outrasQtds + qtd > estoque) {
        mostrarStatus('erro', `Estoque máximo: ${estoque} ${produto.unidade_medida}`);
        return;
      }

      const novo = [...carrinho];
      novo[editIndex] = { ...produto, quantidade: qtd };
      setCarrinho(novo);
    } else {
  ...
  function editarItem(item, idx) {
    setInputQtd(
      item.unidade_medida === 'kg'
        ...
        : String(parseFloat(item.quantidade))
    );
    setItemQuantificar(item);
    setEditIndex(idx);
  }
  ...
  async function finalizarVenda(meioPagamento, clienteId) {
    ...
    setVendaStatus(null);
    try {
      const resp = await fetch(`${API_URL}/api/vendas/finalizar`, {
        method: 'POST',
        ...
        body: JSON.stringify({
          ...
          carrinho: carrinho.map(i => ({
            ...
            preco_venda: parseFloat(i.preco_venda),
          })),
          clienteId,
        }),
      });
      ...
      if (!resp.ok) throw new Error(result.error?.includes('check constraint')
        ? 'Falha de estoque. Verifique as quantidades.'
      ...
      mostrarStatus('erro', `Falha: ${err.message}`);
      setShowPagamento(false);
    } finally {
  ...
  function handleSearchKeyDown(e) {
    if (e.key === 'ArrowDown') {
      e.preventDefault();
      setBuscaIndex(p => Math.min(p + 1, resultados.length - 1));
    } else if (e.key === 'ArrowUp') {
      e.preventDefault();
      setBuscaIndex(p => Math.max(p - 1, 0));
  ...
  function handleSearchSubmit(e) {
    ...
    if (buscaIndex > -1 && resultados[buscaIndex]) {
      selecionarProduto(resultados[buscaIndex]);
  ...
  function estoqueLabel(produto) {
    const e = parseFloat(produto.estoque_atual);
    const u = produto.unidade_medida;
    if (u === 'kg') return `${e.toFixed(3)} kg`;
    return `${Math.trunc(e)} un`;
  }
    ...
    <div className="pdv-container">
      ...
      {itemQuantificar && (
        <div className="pdv-modal-overlay">
          <div className="pdv-modal" onClick={e => e.stopPropagation()}>
            <div className="pdv-modal-qtd-titulo">
              {editIndex !== null ? '✏️ Editar item' : '➕ Adicionar item'}
            </div>
            <div className="pdv-modal-qtd-produto">
              {itemQuantificar.nome}
              {' — '}
              <strong>{fmt(itemQuantificar.preco_venda)}</strong>
              {' / '}
              {itemQuantificar.unidade_medida}
            </div>

            <form onSubmit={confirmarQuantidade}>
              <label className="pdv-modal-qtd-label">
                {itemQuantificar.unidade_medida === 'kg' ? 'Peso (kg)' : 'Quantidade (un)'}
              </label>
              <input
                ref={inputQtdRef}
              ...
              <div className="pdv-modal-acoes">
                <button type="button" className="pdv-modal-btn-cancelar" onClick={fecharModalQtd}>
                  Cancelar (Esc)
                </button>
                <button type="submit" className="pdv-modal-btn-confirmar">
      ...
      <div className="pdv-busca">
        ...
        <ul className="pdv-resultados" ref={resultadosRef}>
          {loadingBusca && (
            <li className="pdv-resultados-status">
              <span>⏳</span>
              Buscando…
            </li>
          )}

          {!loadingBusca && resultados.length === 0 && termoBusca.length > 1 && (
            <li className="pdv-resultados-status">
              <span>🔍</span>
              Nenhum produto encontrado para<br />
          ...
          {resultados.map((p, i) => (
            <li
              key={p.id}
              className={`pdv-produto-card${buscaIndex === i ? ' selecionado' : ''}`}
              onClick={() => selecionarProduto(p)}
              onMouseEnter={() => setBuscaIndex(i)}
            >
              <span className="pdv-card-nome">{p.nome}</span>
              <span className="pdv-card-preco">{fmt(p.preco_venda)}</span>
              <span className={`pdv-card-estoque ${estoqueClass(p)}`}>
                {estoqueLabel(p)}
              </span>
            </li>
          ))}
      ...
      <div className="pdv-carrinho">
        ...
        <div className="pdv-carrinho-header">
          ...
          {carrinho.length > 0 && (
            <span className="pdv-carrinho-count">
        ...
        {vendaStatus && (
          <div className={`pdv-status ${vendaStatus.tipo}`}>
            {vendaStatus.msg}
          </div>
        )}
        ...
        <ul className="pdv-carrinho-lista">
          {carrinho.length === 0 ? (
            <li className="pdv-carrinho-vazio">
              <span className="pdv-carrinho-vazio-icon">🛒</span>
              <p>Carrinho vazio</p>
              <small>Busque e selecione produtos ao lado</small>
            </li>
          ) : (
              ...
              <li key={`${item.id}-${idx}`} className="pdv-item">
                <div className="pdv-item-info" onClick={() => editarItem(item, idx)}>
                  ...
                  <span className="pdv-item-qtde">
                    ...
                    {' @ '}{fmt(item.preco_venda)}
                  </span>
                </div>
                <span className="pdv-item-total">
                  {fmt(item.preco_venda * item.quantidade)}
                </span>
                <button className="pdv-item-remover" onClick={() => removerItem(idx)}>
                  ×
                </button>
              </li>
            ))
